...
def enrolling_moodle():
    ...
    query = f"SELECT * FROM {TABLA_INDICADORES_SERVICIO} WHERE Ejecucion = '{month}-{today.year}' ORDER BY Zona"
    ...
    with open(csv_log, mode='w', newline='') as csv_file, open(txt_log, mode='a') as txt:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(
                requests_for_enrolling_moodle, row[1], row[7]) for row in query_results]
    ...
    upload_local_file(CONNECT_STRING, csv_log, SHARE_NAME, join(
        'servicio', 'logsMoodle', f'logMoodle{today.strftime("%Y-%m-%d-%H")}h.csv'))
...
def send_sms_notifications():
    ...
    with open(csv_log, mode='w', newline='') as csv_file, open(txt_log, mode='a') as txt:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(
                requests_for_sms_notifications, row[1], row[7], row[5], headers, body) for row in query_results]
            for future in concurrent.futures.as_completed(futures):
                try:
                    data = future.result()
    ...
    # Execute uploading function for creating a csv log on file share resource
    upload_local_file(CONNECT_STRING, csv_log, SHARE_NAME, join(
        'servicio', 'logsSMS', f'logAvisosSMS{today.strftime("%Y-%m-%d-%H")}h.csv'))
    # Execute uploading function for creating a log on file share resource
    upload_local_file(CONNECT_STRING, txt_log,
                      SHARE_NAME, 'logMoodleAcciones.txt')
...
def send_email_notifications():
    ...
    query = f"SELECT DISTINCT CorreoElectronico FROM {TABLA_INDICADORES_SERVICIO}"
    ...
    with open(txt_log, mode='a') as txt:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(
                email_csv_report_writer, str(row[1]), row[2], row[3], row[4], row[8], csv_log) for row in query_results]
    ...
    # Execute uploading function for creating a csv log on file share resource
    upload_local_file(CONNECT_STRING, csv_log, SHARE_NAME, join(
        'servicio', 'logsCorreo', f'UReditos_Consolidado_Inscripciones {today.strftime("%Y-%m-%d")}.csv'))
    # Execute uploading function for creating a log on file share resource
    upload_local_file(CONNECT_STRING, txt_log,
                      SHARE_NAME, 'logMoodleAcciones.txt')
